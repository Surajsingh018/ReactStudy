const Employees = () => {
  ...
  return (
    <>
      ...
      {/* emplyess data . user.map  iske andar pura iteration ka code  */}
      {employeesData.users.map((emp) => {
        return(
        <div key={emp.id}>
          <h3>first name: {emp.firstName}</h3>
          <p>Email:{emp.email}</p>
          <p>Age:{emp.age}</p>
          <p>Address:{emp.address.address}</p>
          <p>City:{emp.address.city}</p>
          <p>Hair Color:{emp.hair.color}</p>
          <p>University:{emp.university}</p>
          <p>Crypto Coin:{emp.crypto.coin}</p>
          <p>Eye Color:{emp.eyeColor}</p>
          <p>Coordinates lat:{emp.address.coordinates.lat}</p>
          <p>Postal Code: {emp.address.postalCode}</p>
        </div>
        );
      })}
    </>
